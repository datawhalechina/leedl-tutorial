...
- Pick the best function
	...
	其中$\bar{R}_\theta = \sum_\tau R(\tau)P(\tau|\theta)$。就可以用Gradient Ascent进行最大化，过程为：- 初始化$\theta^0$ - $\theta^1 \leftarrow \theta^0+\eta \triangledown \bar{R}_{\theta^0}$ - $\theta^2 \leftarrow \theta^1+\eta \triangledown \bar{R}_{\theta^1}$ - .......
