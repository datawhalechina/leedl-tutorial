...
假设我们现在做一个调查，调查每个人买公仔的数目。ABCDE代表五个人，A有5个凉宫春，B有4个，C有1个，D有1个，E有0个。A有玉白美琪3个，B有3个，C有1个，D有1个，E有1个。A有小叶撕0个，B有0个，C有0个，D有4个，E有5个。A有小维撕1个，B有1个，C有5个，D有4个，E有4个。你会发现说：在这个matrix上面，每一个table block并不是随机出现的。如果有两个公仔的人，那是因为每个人跟角色的背后是有一些特性的(有一些factor来操控这些特性)
...
那我们现在做一个假设，这个matrix里面的element它都来自于两个vector的inner product。为什么A有5个凉宫春的公仔呢？是因为`$r^A$`跟`$r^1$`的inner product很大(它的inner product是5，所以有5个凉宫春的公仔)，如果`$r^A$`跟`$r^1$`的inner product是4，那B就会有4个凉宫春的公仔，以此类推。
