...
现在 $w_iw_i$ 的更新取决于学习率 ηη ，$x^n_i x_i^n$ 以及上图的紫色划线部分；紫色下划线部分直观上看就是真正的目标 $y^n y^n $与我们的function差距有多大。
...
上一篇讲到如果定义类别1 $y^1,\hat_{y}$，  类别2 $y^1, \hat_{y}$，类别3 $y^1, \hat_{y}$，这样会人为造成类别1 和类型2有一定的关系这种问题。但可以将 y^y^定义为矩阵，这样就避免了。而且为了计算交叉熵，y^y^也需要是个概率分布才可以。
